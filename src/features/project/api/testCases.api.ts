...
export const testCaseApi = {
  ...
  getTestCasesByRequirement: async (reqId: string): Promise<TestCase[]> => {
    const response = await httpClient.get(`/core-managerment-service/api/test-cases/by-requirement/${reqId}`);
    return response.data;
  },
  ...
  getTestCaseTestData: async (id: number): Promise<any[]> => {
    const response = await httpClient.get(`/core-managerment-service/api/test-cases/${id}/test-data`);
    return response.data;
  },
  ...
  generateScript: async (tcId: number, baseUrl: string, scriptLanguage: string = 'javascript', framework: string = 'playwright'): Promise<any> => {
    const response = await httpClient.post(`/core-managerment-service/api/test-cases/${tcId}/generate-script`, {
      baseUrl,
      scriptLanguage,
      framework
    });
  ...
  executeScript: async (tcId: number, baseUrl: string, maxFixAttempts?: number): Promise<any> => {
    const response = await httpClient.post(`/core-managerment-service/api/test-cases/${tcId}/execute`, {
      baseUrl,
      maxFixAttempts
    });
    return response.data;
