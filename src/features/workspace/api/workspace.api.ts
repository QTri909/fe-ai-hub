...
export const workspaceApi = {
  getWorkspaces: async (page = 0, size = 10): Promise<Page<Workspace>> => {
    const response = await httpClient.get<Page<Workspace>>('/core-management-service/api/v1/workspaces', {
      params: { page, size }
    });
  ...
  getWorkspaceById: async (id: string): Promise<Workspace> => {
    const response = await httpClient.get<Workspace>(`/core-management-service/api/v1/workspaces/${id}`);
    return response.data;
  },
  ...
  createWorkspace: async (data: { name: string; description: string; jiraUrl: string; email: string; apiToken: string; ownerId: string }): Promise<Workspace> => {
    const response = await httpClient.post<Workspace>('/core-management-service/api/v1/workspaces', data);
    return response.data;
  }
};
